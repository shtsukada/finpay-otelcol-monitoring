...
現在のMVP監視スタックとしては `otelcol / prometheus / tempo / grafana / loki` までを実装済みです。
`finpayApi` / `finpayClient` / `redis` は values に定義されていますが、規定では無効であり、監視デモの種経路には含めていません。
...
* Runbook: `docs/runbooks/loss-alerts.md`
...
* otelcol-gateway health: [http://localhost:13133](http://localhost:13133/healthz)
...
* さらにrecording ruleが`rate(...[1m])`を使うため、値が揺れたり空になることがあります
